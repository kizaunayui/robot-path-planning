...
export default function MapOverview() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-2xl font-bold text-slate-800">🗺️ 地图总览</h2>
      ...
      <div className="grid grid-cols-4 gap-3">
        {Object.entries(map.points).map(([name, pos]) => (
          <div key={name} className="border border-slate-200 rounded p-3 text-sm bg-white" style={{ borderLeftColor: pointColors[name], borderLeftWidth: 4 }}>
            <div className="font-semibold text-slate-700">
              {pointIcons[name]} {name}
            </div>
            <div className="text-slate-500 text-xs">坐标: ({pos[0]}, {pos[1]})</div>
          </div>
        ))}
      </div>
      ...
      {/* Robot status */}
      <div>
        <h3 className="font-bold mb-2 text-slate-700">🤖 机器人状态</h3>
        <div className="grid grid-cols-3 gap-2">
          {robots.map((r) => (
            <div key={r.id} className="border border-slate-200 rounded p-2 text-sm bg-white">
              <div className="font-semibold text-slate-700">{r.name}</div>
              <div className="text-slate-500 text-xs">{r.id}</div>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-slate-500">🔋 {r.battery}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Active rules */}
      <div>
        <h3 className="font-bold mb-2 text-slate-700">🚦 生效中的规则</h3>
        <div className="flex flex-wrap gap-2">
          {rules.filter((r) => r.enabled).map((rule) => (
            <span key={rule.id} className="bg-slate-100 text-slate-700 px-3 py-1 rounded text-xs border border-slate-200">
