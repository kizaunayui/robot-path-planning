    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {Object.values(AREA_TYPES).map(area => (
          <div key={area.id} className="border rounded p-3 text-sm" style={{ backgroundColor: area.color + '40' }}>
            <div className="font-semibold">{area.name}</div>
            <div className="text-gray-600">路径代价: {area.cost}</div>
            <div className="text-gray-600">缓冲距离: {area.buffer}m</div>
          </div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {robots.map(r => (
            <div key={r.id} className="border rounded p-2 text-sm">
              ...
              <div className="text-gray-500">{r.id} | {r.type}</div>
