...
export default function MultiRobot() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-2xl font-bold text-slate-800">🤖 多机器人协调</h2>
      ...
      <div className="flex gap-4 items-start">
        ...
        <div className="flex-1 bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {robots.map((r) => (
              <div key={r.id} className="p-2 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500">
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded text-xs ${
                      r.status === "running" ? "bg-green-100 text-green-700" : r.status === "charging" ? "bg-yellow-100 text-yellow-700" : "bg-slate-100 text-slate-500"
                    }`}
                  >
          ...
          {/* Traffic signals */}
          <div>
            <h4 className="text-xs font-bold text-slate-600 mb-2">🚦 交通信号状态</h4>
            <div className="grid grid-cols-2 gap-2">
              {signals.map((sig) => (
                <div key={sig.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg border border-slate-200">
                  <div>
                    <div className="text-xs font-medium text-slate-700">{sig.location}</div>
        ...
        <div className="w-96 space-y-3">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
            <h3 className="text-sm font-bold text-slate-700 mb-3">冲突预测</h3>
            <div className="space-y-1.5 max-h-64 overflow-y-auto">
              {conflicts.map((cp) => (
                <div
                  key={cp.id}
                  className={`p-2 rounded-lg transition-all ${cp.resolved ? "bg-slate-50 opacity-60" : "bg-white border border-slate-200"}`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-slate-700 font-medium">{cp.type}</span>
                    <span
