  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        {rules.map((rule) => {
          ...
          return (
            <div
              key={rule.id}
              className={`bg-slate-900 border rounded-lg p-4 transition ${
                rule.enabled ? "border-slate-700/60" : "border-slate-800 opacity-60"
              }`}
            >
              ...
              <div className="mt-2">
                <span className={`text-xs px-2 py-0.5 rounded ${rule.enabled ? "bg-green-500/20 text-green-400" : "bg-slate-800 text-slate-600"}`}>
          ...
          <div className="grid grid-cols-4 gap-4 text-center">
            <div>
              ...
              <div className="text-lg font-bold text-white">
                {beforeBest.length} <ArrowRight className="w-4 h-4 inline text-slate-600" /> {afterBest.length}
              </div>
              <div className={`text-xs ${afterBest.length < beforeBest.length ? "text-green-400" : afterBest.length > beforeBest.length ? "text-red-400" : "text-slate-500"}`}>
                {afterBest.length - beforeBest.length > 0 ? "+" : ""}{afterBest.length - beforeBest.length} 步
              </div>
            </div>
            <div>
              <div className="text-xs text-slate-500 mb-1">转弯次数</div>
              <div className="text-lg font-bold text-white">
                {beforeBest.turns} <ArrowRight className="w-4 h-4 inline text-slate-600" /> {afterBest.turns}
              </div>
            </div>
            <div>
              <div className="text-xs text-slate-500 mb-1">预计耗时</div>
              <div className="text-lg font-bold text-white">
                {beforeBest.estimatedMinutes} <ArrowRight className="w-4 h-4 inline text-slate-600" /> {afterBest.estimatedMinutes}
              </div>
            </div>
            <div>
              <div className="text-xs text-slate-500 mb-1">综合评分</div>
              <div className="text-lg font-bold text-white">
                {beforeBest.score} <ArrowRight className="w-4 h-4 inline text-slate-600" /> {afterBest.score}
              </div>
            </div>
          </div>
          <button
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <StatCard label="可达路径" value={routes.filter((r) => r.reachable).length} icon={Download} color="text-purple-400" />
      </div>
