...
export default function NavMesh() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-2xl font-bold text-slate-800">🔺 导航网格</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-slate-700">地图网格</h3>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-700 mb-3">网格统计</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-500">网格尺寸</span>
              <span className="text-slate-700 font-mono">{map.cols} × {map.rows}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">总单元数</span>
              <span className="text-slate-700 font-mono">{map.cols * map.rows}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">墙壁数量</span>
              <span className="text-slate-700 font-mono">{map.walls.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">动态障碍</span>
              <span className="text-slate-700 font-mono">{map.dynamic.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">科室节点</span>
              <span className="text-slate-700 font-mono">{Object.keys(map.points).length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">可通行率</span>
              <span className="text-green-600 font-mono">{coverage}%</span>
            </div>
          </div>

          <div className="mt-4 pt-3 border-t border-slate-200">
            <h4 className="text-xs font-bold text-slate-600 mb-2">机器人分布</h4>
