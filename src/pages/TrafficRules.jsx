  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-100">🚦 交通规则管理</h2>
      ...
      <div className="grid grid-cols-2 gap-4">
        {rules.map((rule) => {
          ...
          return (
            <div
              key={rule.id}
              className={`bg-slate-800 border-l-4 rounded-lg p-4 shadow-sm transition ${
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    isActive ? "bg-green-100 text-green-700" : "bg-slate-700 text-slate-400"
                  }`}
                >
                  {isActive ? "已启用" : "已停用"}
                </span>
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 rounded text-xs bg-slate-700 text-slate-400 hover:bg-slate-200"
                >
                  删除
                </button>
              </div>
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 p-5 shadow-sm">
        <h3 className="text-sm font-bold text-slate-200 mb-3">⚙️ 路径参数</h3>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-xs text-slate-400 mb-1 block">灵敏度 ({params.sensitivity})</label>
            <input
              type="range"
              min={1}
              max={5}
          ...
          <div>
            <label className="text-xs text-slate-400 mb-1 block">缓冲距离 ({params.buffer})</label>
            <input
              type="range"
              min={0}
              max={3}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-lg p-6 w-[420px] shadow-xl">
            <h3 className="text-lg font-bold text-slate-100 mb-4">添加交通规则</h3>
            <div className="space-y-3">
              <div>
                <label className="text-xs text-slate-400 mb-1 block">规则类型</label>
                  ...
                  {Object.entries(RULE_TYPE_INFO).map(([type, info]) => (
                    <button
                      ...
                      className={`p-2 rounded text-xs text-center border transition ${
                        newRule.type === type ? "border-blue-500 bg-blue-50" : "border-slate-700 hover:border-slate-600"
                      }`}
                    >
              ...
              <div>
                <label className="text-xs text-slate-400 mb-1 block">权重 ({newRule.weight})</label>
                <input
                  type="range"
                  min={0.5}
                  max={3}
            ...
            <div className="flex justify-end gap-3 mt-5">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 rounded text-sm bg-slate-700 hover:bg-slate-200">
                取消
              </button>
